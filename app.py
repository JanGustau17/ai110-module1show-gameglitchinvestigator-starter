...
def get_range_for_difficulty(difficulty: str):
    ...
    if difficulty == "Normal":
        return 1, 100
    if difficulty == "Hard":
        return 1, 200  # BUG FIX: was 1-50 (easier than Normal's 1-100); Hard should have a larger range
    return 1, 100
...
def check_guess(guess, secret):
    ...
    try:
        if guess > secret:
            return "Too High", "📉 Go LOWER!"  # BUG FIX: was "Go HIGHER!" — guess is too high so player should go lower
        else:
            return "Too Low", "📈 Go HIGHER!"  # BUG FIX: was "Go LOWER!" — guess is too low so player should go higher
    except TypeError:
        g = str(guess)
        if g == secret:
            return "Win", "🎉 Correct!"
        if g > secret:
            return "Too High", "📉 Go LOWER!"  # BUG FIX: same inversion in fallback
        return "Too Low", "📈 Go HIGHER!"      # BUG FIX: same inversion in fallback
...
def update_score(current_score: int, outcome: str, attempt_number: int):
    ...
    if outcome == "Too High":
        # BUG FIX: was giving +5 points on even attempts for a wrong guess; wrong guesses should always subtract
